...
public class UsuarioResourceTest {
	...
	public void deveAutenticarUmUsuario() throws Exception {
		...
		MockHttpServletRequestBuilder request = MockMvcRequestBuilders
			.post( API.concat("/autenticar") ).accept( JSON ).contentType( JSON ).content(json);
	...
	public void deveObterOSaldoDoUsuario() throws Exception {
		...
		Mockito.when(service.obterUsuarioPorId(1l)).thenReturn(Optional.of(usuario));
		Mockito.when(lancamentoService.obterSaldoPorUsuario(1l)).thenReturn(saldo);
		
		//execucao e verificacao
		MockHttpServletRequestBuilder request = MockMvcRequestBuilders
			.get( API.concat("/1/saldo")).accept( JSON ).contentType( JSON );
	...
	public void deveRetornarResourceNotFoundQuandoUsuarioNaoExisteParaObterOSaldo() throws Exception {
		//cenário
		Mockito.when(service.obterUsuarioPorId(1l)).thenReturn(Optional.empty());
